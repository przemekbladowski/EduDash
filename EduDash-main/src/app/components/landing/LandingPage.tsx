  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-white font-sans" style={{ fontFamily: "'Inter', system-ui, sans-serif" }}>
        ...
        <a href="#funkcje" className="sr-only focus:not-sr-only focus:fixed focus:top-2 focus:left-2 focus:z-[100] focus:bg-indigo-600 focus:text-white focus:px-4 focus:py-2 focus:rounded-xl focus:text-sm focus:font-semibold">
          Przejdź do treści
        </a>
              ...
              <form onSubmit={handleFormSubmit} className="bg-white/5 border border-white/10 rounded-2xl p-6 md:p-8 space-y-4">
                <div className="grid sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input required id="contact-name" type="text" placeholder="Jan Kowalski" autoComplete="name"
                      value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })}
                      className="w-full bg-white/10 border border-white/20 rounded-xl px-4 py-3 text-white text-sm placeholder-slate-500 outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-400/30 transition-colors" />
                  </div>
                  <div>
                    <label className="block text-slate-300 text-xs font-medium mb-1.5">Adres e-mail *</label>
                    <input required id="contact-email" type="email" placeholder="dyrektor@szkola.pl" autoComplete="email"
